    ...
    def ModTrace(self, traceitem: QtWidgets.QListWidgetItem):
        try:
            ...
            if isinstance(selectedtrace.reader, TFDataReader):
                ...
                # ! ACA NO ES UN TFDataREader
                popup = ModTracePopUp(selectedtrace.tracename, selectedtrace.color, selectedtrace.marker,
                                      selectedtrace.linetype, selectedtrace.reader.filterCoeff)
                update = popup.exec()
                for i in range(len(update)):
                    # ? MODIFICAR ACA????
                    if isinstance(update[i], int):
                        selectedtrace.reader.filterCoeff = update[i]
                        logging.debug(f"Filter Coef >:D {selectedtrace.reader.filterCoeff}")
                        continue
                    elif update[i] != "":
                        match i:
                            case 0:
