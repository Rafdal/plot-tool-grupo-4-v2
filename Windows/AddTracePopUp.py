...
class YAxisDataWidget:  # ! Rectangulo gris por cada eje Y al agregar una señal
...
class AddTracePopUp(QtWidgets.QDialog, UI_AddTracePopUp.Ui_AddTracePopUp):
    def __init__(self):
        ...
        self.FileDropBox.setAcceptDrops(True)
        self.label.setAcceptDrops(True)
        self.FileDropBox.dragEnterEvent = self.dragEnterEvent
        self.label.dragEnterEvent = self.dragEnterEvent
        self.FileDropBox.dropEvent = self.dropEvent
    ...
    def dropEvent(self, event):
        if event.mimeData().hasUrls():
            ...
            logging.info(f"File path: {file_path[-3:]}")
